...
func main() {
	...
	for i := 0; i < 2; i++ {
		select {
		case msg := <-tower1Chan:
			fmt.Printf("\n\nControl Tower: Message from Tower 1 - %v\n", msg)
		case msg := <-tower2Chan:
			fmt.Printf("\nControl Tower: Message from Tower 2 - %v\n", msg)
		}
	}
}
...
func tower1(s chan string, t1 chan string, offset int32) {
	...
	fmt.Printf("Tower 1: Encrypted String: %s", secretString)
...
func tower2(s chan string, t2 chan string, offset int32) {
	...
	fmt.Printf("\nTower2: Encrypted Message: %s", secretString)
